...
const TEMPLATE_SPEC = 'ggoodman/fringe/packages/template#main'
...
function init() {
  Yargs.usage(
    '$0 [dest]',
    'Scaffold a Fringe App into the target directory.',
    yargs =>
      yargs
        .positional('dest', {
        ...
        .options({
          force: {
            boolean: true,
            description:
              'Force scaffolding even if the target directory is not empty',
          },
        }),
    async argv => {
      try {
        ...
        const packageJsonHandle = await Fs.open(
        ...
        console.error(Chalk.green('- Your Fringe project is now ready'))
      } catch (err) {
